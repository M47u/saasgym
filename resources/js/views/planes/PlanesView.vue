...
function preciosLabel(plan) {
    const ef = formatPrecio(plan.precio_efectivo);
    const di = formatPrecio(plan.precio_digital);
    if (!ef && !di) return null;
    if (ef && di && ef !== di) return { efectivo: ef, digital: di };
    return { unico: ef || di };
}
</script>
            ...
            <div class="divide-y divide-gray-50">
                ...
                >
                    ...
                    <div class="hidden sm:flex items-center gap-2 shrink-0">
                        <template v-if="preciosLabel(plan)">
                            <template v-if="preciosLabel(plan).unico">
                                <span class="text-sm font-semibold text-gray-900">{{ preciosLabel(plan).unico }}</span>
                            </template>
                            <template v-else>
